...
#include <arpa/inet.h>
#include "third_party/include/gzip.h"
				 ...
				 struct ndpi_flow_struct *flow)
{
  ...
  for (a = 0; a < end; a++) {
	...
	is_gzip = 1;
      ...
      if(packet->line[packet->parsed_lines].len > 6
	 &&memcmp(packet->line[packet->parsed_lines].ptr, "Date: ", 6) == 0)
      {
	packet->http_date.ptr = &packet->line[packet->parsed_lines].ptr[6];
	packet->http_date.len = packet->line[packet->parsed_lines].len - 6;
	strncpy(packet->http_Date,packet->http_date.ptr,packet->http_date.len);  
      }
	...
	interger = atoi(packet->http_contentlen.ptr);
	if(interger>1460) interger=1460;
      } 
      if(packet->line[packet->parsed_lines].len > 8
