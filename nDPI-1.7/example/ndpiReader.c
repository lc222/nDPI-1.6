...
typedef struct ndpi_flow {
  ...
  //char http_sourcecode[655350];
  int count;
  int inter;
  char http_code [65530];
  int is_gzip;
  int is_chunked;
  u_int32_t seq;
...
void *memstr1(void *src, size_t src_len, char *sub)
{
...
char * strget(char * src, const char *str1, const char *str2, char * dest)
{
  char st [100] = {0};
  dest = st;
  char * ss = strstr(src, str1);
  char * tt = strstr(src, str2);
  if(ss != NULL && tt != NULL)
  {
	int aa = strlen(ss);
	int bb = strlen(tt);
	strncpy(dest, &ss[strlen(str1)], aa-bb-strlen(str1));
	return dest;
  }
  return NULL;
}
...
static struct ndpi_flow *get_ndpi_flow(u_int16_t thread_id,
  ...
  *proto = iph->protocol;
  ...
  if(iph->protocol == 6 && l4_packet_len >= 20) {
	...
	{
		payload = (u_int8_t *)tcph + tcph->doff * 4;
		if(payload[0] != 0)
		   is_80 = 1;
	}
  ...
  if(ret == NULL) {
    ...
    if(is_80 == 1)
	{
	  //用tcp->seq来指定每个数据包要存放的位置，实现数据包的排序与重组
	  char * t = strstr((char *)payload , "Content-Encoding: ");
	  char * tt = strstr((char *)payload , "Content-Length: ");
	  char * ttt = strstr((char *)payload , "Transfer-Encoding: ");
	  if(t != NULL)
	  {
	    if(tt != NULL)
	    {
		char * ss = strstr(payload , "\r\n\r\n");
		char * sss = & ss[4];
		ungzip(sss, 1460, flow->http_code);
		char * st = strstr(flow->http_code, "<title>");
		char * stt = strstr(flow->http_code, "</title>");
		if(st != NULL && stt != NULL)
		{
			int aa = strlen(st);
			int bb = strlen(stt);
			char dest [1000] = {0};
			strncpy(dest, &st[7], aa-bb-7);
			printf("gzip %s\n", dest);
		}
	    }
	    else if (ttt !=NULL)
...
static unsigned int packet_processing(u_int16_t thread_id,
  ...
  if((flow->detected_protocol != NDPI_PROTOCOL_UNKNOWN)
	...
	|| (flow->detected_protocol == NDPI_SERVICE_FACEBOOK)
       ...
       && full_http_dissection && ndpi_flow->packet.is_http_content == 1){
      char *method;	
